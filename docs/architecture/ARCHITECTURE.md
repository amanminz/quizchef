...
**Feature modules** (`features/<name>/`, established by quiz authoring, Phase 2 PR #2): each owns its query hooks, its orchestration hooks (the coordination layer a page actually calls — e.g. `useQuizAuthoring`, `useQuestionSelection`, `useQuizPublishing`), its presentational components, and a `queryKeys.ts` registry so a mutation's cache invalidation can never drift from a query's key. `components/` and the cross-cutting `hooks/` stay feature-ignorant by rule; anything that knows about quizzes lives in `features/quizzes/`. `features/questions/` (Phase 3 PR #4A, RFC-013) owns question authoring — the one `QuestionEditor` used standalone and quiz-launched, the question query/mutation hooks and `questionKeys` (moved from `features/quizzes/`, whose picker was the only consumer before authoring existed) — while composition (attach/detach/reorder) stays with the quiz aggregate's feature in `useQuestionSelection`.
...
**Host and participant projections are role-scoped** (Live Event UX, Phase 3): the host's screens read the full standings (`useResults` → host-only `GET /results`) and the lobby roster wall (`useRoster` → host-only `GET /participants`); a participant device reads only its own row (`useParticipantResult` → `GET /participants/{id}/result`) and never mounts — or is even authorized for — the full-leaderboard query. Realtime result events invalidate the role's own query only, and the `leaderboard.updated` broadcast carries no standings (pure notification). Presentation mode (chrome-free projected layout + browser fullscreen + screen wake lock) is local UI state in a sessionStorage-persisted Zustand store; the layout hides chrome with CSS on a stable tree — a branch swap would remount the page and bounce its realtime connection mid-event. The podium reveal is local, skippable display state that plays once per session (sessionStorage) and collapses to the finished podium under `prefers-reduced-motion`.
