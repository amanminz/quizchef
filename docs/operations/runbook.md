...
2. `Enter Presentation Mode` (button, host lobby/live screens) — chrome hides and the browser requests fullscreen; if the browser refuses, the layout still applies and a hint suggests the browser's own fullscreen shortcut. A screen wake lock keeps the projector awake where supported. The live-question view fits one screen at common projector resolutions (1366×768, 1920×1080) with no scrolling — a compact `Answered`/`Time left` row replaces the old full-size timer block; if a question still overflows on an unusual display, that's a bug, not expected behavior.
...
7. On the *last* question specifically: once its leaderboard step shows, every participant device already switches to "Quiz complete! The winners are being announced" — before you've clicked `Finish Quiz`, and well before the podium. This is intentional (the final-rank hold applies from that moment, not only once the session is technically finished) — it is not a bug if a participant reports seeing the waiting screen slightly early.
8. At the finish, the podium reveals 5th → 4th → 3rd → 2nd → 1st (Skip available; Replay is display-only). A refresh renders the completed results from the backend — it never replays commands. Participants keep seeing the waiting screen throughout the ceremony; only `Reveal Results to Participants` releases their final rank.
