...
`OptionCount.percentage` is `round(count / answeredCount * 100)` — a share of *accepted answers*, deliberately never of `eligibleParticipantCount`. Dividing by eligible would understate every option's share whenever some eligible participant simply never answered, which is the common case, not the exception. For a single/true-false question this makes `sum(options[].percentage) ≈ 100`; for a multiple-answer question, `sum(options[].count)` (and therefore `sum(options[].percentage)`) may legitimately exceed `answeredCount` (100) by design — one participant, several selections — documented on the view and the response schema. `noAnswerCount` is exactly `eligibleParticipantCount - answeredCount`.
