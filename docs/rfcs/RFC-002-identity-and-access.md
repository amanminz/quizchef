...
- **A richer permission model** (Aman, Phase 3 PR #1 review): role → permissions → namespaced capabilities (`quiz:create`, `quiz:publish`, `session:host`, `session:moderate`, `user:promote`) instead of the coarse enum, likely persisted rather than code-defined. The architecture is already shaped for it — deliberately, so this stays an evolution, not a refactor: **no business code checks roles** (every decision is `authorize(currentUser, permission)` through the one `AuthorizationService`; verified — the only `.roles()` outside this module is the security adapter's principal mapping), the `Permission` enum is the capability vocabulary in all but spelling, `RolePermissions` is the single mapping to swap for a persisted policy, and the frontend checks `hasPermission`, not roles (its one `isHost` derivation is cosmetic onboarding UX). The dotted wire names can ride the same `@JsonValue` trick as `ProtocolMessageType` without renaming the enum internally. A `user:promote` capability is also what turns today's self-service promotion into an admin-approved gate with no API change.
