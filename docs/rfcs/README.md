...
- [RFC-017 Compact Projector Layout and the Final-Rank Hold](RFC-017-projector-layout-and-final-rank-hold.md) — **Implemented** (Presentation Mode fits one projector screen via a shared compact metric primitive; closes two participant-facing rank leaks — the last question's pre-FINISHED window, and an unconditional full-leaderboard leak in the reconnect snapshot; Question Versioning still deferred to PR #47B)
- [RFC-018 The Animated Top 5 and the Final Question's Missing Leaderboard](RFC-018-animated-top-five-leaderboard.md) — **Implemented** (host-only before/after Top 5 projection with `LeaderboardService.rankBefore`; score count-up then rank movement, skippable and reduced-motion aware; the quiz's last question has no leaderboard step at all, refused in the engine; Question Versioning still deferred to PR #47B)
- [RFC-019 Nobody Learns They Came Last](RFC-019-inclusive-results-and-suspense.md) — **Implemented** (participant ranks removed from the wire during play — not hidden, not computed; localized encouragement catalogue; `FinalPlacementPolicy` splits the finish into an exact-rank reveal group and relative-only context for everyone else, one cutoff shared by the projector and the phones; rank-context endpoint deleted; Question Versioning still deferred to PR #47B)
- [RFC-021 Resuming an Anonymous Participant](RFC-021-participant-resume-and-name-reservation.md) — **Implemented** (resume proved by a server-issued token, never a display name; the token stored hashed and scoped to one session; resume addressed by PIN so a credential from an archived session cannot restore a player into the quiz reusing its code; display-name reservation per session with the resume path exempt; amends RFC-004's reconnect endpoint and ADR-003's guest credential; amended 2026-08-17 with host-assisted recovery codes and the three carrying-out defects that stranded a live event — dropped storage key, resume falling through to join, and a rate-limit rule left naming the renamed route)
- [RFC-020 Correcting and Removing a Question During a Live Session](RFC-020-live-question-correction-and-removal.md) — **Implemented** (session-scoped correction and removal, never touching the Question Library; `SessionQuizQuery` is the one effective-quiz boundary so numbering closes the gap with no arithmetic anywhere; backend-authoritative reversal of answers and points; row-locked against a concurrent answer; additive `question.corrected`/`question.removed` events carrying ids only; Question Versioning — "save this correction as a new published version" — still deferred to PR #47B)
